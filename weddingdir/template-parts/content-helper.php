 ...
 *  -----------------
 */
if( ! class_exists( 'WeddingDir_Blog_Helper' ) && class_exists( 'WeddingDir' ) ){

	/**
	 *  WeddingDir - Blog
	 *  -----------------
	 */
	class WeddingDir_Blog_Helper extends WeddingDir {
		...
		public function __construct(){
			...
			/**
			 *  WeddingDir - Media Data
			 *  -----------------------
			 */
			add_filter( 'weddingdir/media-data', [ $this, 'weddingdir_media' ], absint( '1' ), absint( '1' ) );
		}
		...
		public static function is_elementor_edit_mode(){
			...
			/**
			 *  Have Elementor Plugin Activated ?
			 *  ---------------------------------
			 */
			if( defined( 'ELEMENTOR_VERSION' ) ){

				$_condition_1 	=	\Elementor\Plugin::$instance->preview->is_preview_mode();

				$_condition_2 	=	\Elementor\Plugin::$instance->editor->is_edit_mode();
		...
		/** 
		 * 	WeddingDir - The Excerpt for word counting in the_content
		 * 	---------------------------------------------------------
		 */
		public static function WeddingDir_getwords( $text, $limit ) {
		...
		/**
		 *  WeddingDir - Post Format Load Condition
		 *  ---------------------------------------
		 */
		public static function post_load_condition( $post_id = '0' ){
    		 ...
    		 *  -----------------------------
    		 */
    		return 	$_have_post_format &&  $_weddingdir_posts;
		}

		/** 
		 * 	WeddingDir - The Excerpt for Character counting in the_content
		 * 	--------------------------------------------------------------
		 */
		public static function weddingdir_getcharcut(  $text,  $limit  ){
		 ...
		 *  -----------------------
		 */
		public static function weddingdir_blog_tags(){

			/**
			 *  WeddingDir - Post Have Tags ?
			 *  -----------------------------
			 */
			if( has_tag() ){

				/**
				 *  WeddingDir - Load the Tags
				 *  --------------------------
				...
				the_tags( 
					 ...
					 *  --------------------
					 */
					'<div class="weddingdir-section-post-tags tag-wrap">
		...
		public static function post_tags(){
			...
			if( has_tag() ){
				...
				?>
					<div class="weddingdir-theme-tags">
						<?php the_tags( '<div class="widget-tags"> <h5>'. esc_attr__( 'Tags:', 'weddingdir' ) .'</h5> ', '', '</div>' ); ?>
					</div>
		...
		public static function date_formate( $post_id = '0' ){
			...
			sprintf(   '<span class="meta-date">
					 ...
					 *  ------------
					 */
					esc_attr( get_the_date( 'M d, Y', absint( $post_id ) ) ),
		...
		public static function weddingdir_entry_content_markup_layout_two(){
			...
			?>
				...
				<div class="blog-wrap-home">
				    ...
				    <div class="post-content">
				        ...
				        <?php echo self:: post_formate( 'weddingdir_blog_360x480' ); ?>
		 ...
		 *  ----------------
		 */
		public static function weddingdir_empty_article_markup(){
		 ...
		 *  ----------
		 */
		public static function weddingdir_post_class( $args = [] ){
			 ...
			 *  -------------------
			 */
			$classes[] = 	sanitize_html_class( 'weddingdir-blog-style-' . absint( $layout ) );

			/**
			 *  Common prefix class
			 *  -------------------
			 */
			$classes[] = 	sanitize_html_class( 'weddingdir-section-post-content' );
			...
			if( is_single() ){
				...
				$classes[] = 	sanitize_html_class( 'weddingdir-singular-page' );
			}
		...
		public static function post_category( $post_id = '0' ){
			...
			if ( ! empty( $categories ) ) {
			    ...
			    foreach( $categories as $category ) {
			        ...
			        sprintf( '<a href="%1$s" title="%2$s">%3$s</a> %4$s', 
			        	 ...
			        	 *  ----------------
			        	 */
						esc_attr( sprintf( __( 'View all posts in %s', 'weddingdir' ), $category->name ) ),
			...
			if( $output !== '' && $output !== NULL &&  ! empty( $output ) ){
				...
				sprintf( '<span class="post-category">%1$s</span>',

					/**
					 *  1. Total Category with Seperator
					 *  --------------------------------
					 */
		 ...
		 *  ---------
		 */
		public static function weddingdir_content_edit_post(){
		 ...
		 */
		public static function weddingdir_content_comment_e(){

			echo self:: weddingdir_content_comment();
		}
		 ...
		 *  ------------
		 */
		public static function weddingdir_content_comment(){
		...
		/**
		 *  -----------------------------------------------
		 *  WeddingDir - Get Media Id to retrive image link
		 *  -----------------------------------------------
		 *  @credit - https://developer.wordpress.org/reference/functions/wp_get_attachment_image_src/
		 *  ------------------------------------------------------------------------------------------
		 */
		public static function weddingdir_media( $args = [] ){
		 ...
		 *  -------------------
		 */
		public static function weddingdir_image( $args = [] ){
		...
		public static function is_image_post( $args = [] ){
			...
			if(  parent:: _is_array( $args )  ){
				...
				if (   has_post_thumbnail( $post_id )   ) {
					 ...
					 *  -----------------
					 */
					$_custom_image 		=		self:: weddingdir_image( array(
					...
					$output 	.= 		sprintf( '<div class="post-img">
							...
							if( is_single() ){
								...
								$output 		.=		self:: weddingdir_image( array(
			...
			if(  parent:: _is_array( $args )  ){
				...
				if(  $_condition_1 && $_condition_2  ){
					...
					$output 	.=	'<div class="post-img post-type-slider mb-4">
										...
										<div class="weddingdir-post-slider owl-carousel owl-theme">';
		...
		public static function is_video_post( $args = [] ){
			...
			if(  parent:: _is_array( $args )  ){
				...
				if (  parent:: _have_data( $_have_video )  ) {
					...
					$output 	.=		sprintf(   '<div class="weddingdir-post-formate embed-responsive embed-responsive-16by9 mb-4">
		...
		public static function post_formate( $args = [] ){
			...
			if( parent:: _is_array( $args ) ){
				...
				/**  
				 *   WeddingDir - Post Format Custom HTML Return
				 *   -------------------------------------------
				 */
		...
		public static function weddingdir_video_embed( $embed_code ){
		...
		/**
		 *  ------------------------------------------------
		 *  WeddingDir - Next and Prev Link on Singular page
		 *  ------------------------------------------------
		 *  @credit - https://developer.wordpress.org/reference/functions/get_adjacent_post/
		 *  --------------------------------------------------------------------------------
		 */
		public static function weddingdir_single_post_link(){
			
			/**
			 *  WeddingDir - Prev Post
			 *  ----------------------
			 */
			$weddingdir_prev_post 	= 	get_adjacent_post( false, '', true );

			/**
			 *  WeddingDir - Next Post
			 *  ----------------------
			 */
			$weddingdir_next_post 	= 	get_adjacent_post( false, '', false );
			...
			if( ! empty( $weddingdir_prev_post ) || ! empty( $weddingdir_next_post ) ) {
	    	    ...
	    	    /** 
	    	     *   WeddingDir - Previous Post Title + Link
	    	     *   ---------------------------------------
	    	     */
		        if( ! 	empty( $weddingdir_prev_post ) 	){
					...
					sprintf('	<div class="previous-post">
							...
							esc_url( 
								...
								get_permalink( 
									 ...
									 *  -------
									 */
									absint( $weddingdir_prev_post->ID ) 
								)
							),
							 ...
							 *  ---------------------------
							 */
							esc_attr__( 'Previous Post', 'weddingdir' )
					);
		        }

	    	    /** 
	    	     *   WeddingDir - Next Post Title + Link
	    	     *   -----------------------------------
	    	     */
		        if( ! 	empty( $weddingdir_next_post ) 	){
					...
					sprintf('	<div class="next-post">
							...
							esc_url( 
								...
								get_permalink( 
									 ...
									 *  -------
									 */
									absint( $weddingdir_next_post->ID ) 
								)
							),
							 ...
							 *  ---------------------------
							 */
							esc_attr__( 'Next Post', 'weddingdir' )
					);
		        }

		        /**
		         *  Load : WeddingDir - Next & Prev Section
		         *  ---------------------------------------
		         */
		        printf( '<div class="weddingdir-section-post-next-prev post-linking %3$s %4$s">%1$s %2$s </div>',
		...
		public static function read_more( $args = [] ){
			...
			if( parent:: _is_array( $args ) ){
				 ...
				 *  ------------------------
				 */
				$_read_more 	=	esc_attr__( 'Read More', 'weddingdir' );
	 ...
	 *  -----------------------------
	 */
	WeddingDir_Blog_Helper::get_instance();
}