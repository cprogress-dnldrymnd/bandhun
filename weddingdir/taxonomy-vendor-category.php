...
if( class_exists( 'WeddingDir_Loader' ) ) :
	 ...
	 *  -------------
	 */
	$_taxonomy_slug 	=	esc_attr( WeddingDir_Taxonomy_Database:: vendor_category_taxonomy() );
	...
	if( class_exists( 'WeddingDir_Loader' ) ){
        ...
        if( $item->have_posts() ){
            ...
            while ( $item->have_posts() ){  $item->the_post();
                ...
                if( class_exists( 'WeddingDir_Vendor' ) ){

                    $_listing_html_data[] =

                    sprintf( '<div class="col-md-4 col-sm-6 col-12">%1$s</div>',

                    	/**
                    	 *  1. Vendor Layout 1
                    	 *  ------------------
                    	 */
                        WeddingDir_Vendor:: weddingdir_vendor_layout( array(
